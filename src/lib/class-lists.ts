...
import { cloudDeleteTeacherClassList, cloudLoadTeacherClassListsByOwner, cloudUpsertTeacherClassList } from "./class-lists-cloud";
...
export function loadTeacherClassLists() {
  ...
  try {
    const parsed = repairMojibakeDeep(JSON.parse(raw)) as TeacherClassList[];
...
export function upsertTeacherClassList(payload: TeacherClassList) {
  ...
  void cloudUpsertTeacherClassList(normalized);
  return normalized;
}
...
export function deleteTeacherClassList(classListId: string) {
  ...
  saveTeacherClassLists(nextClassLists);
  void cloudDeleteTeacherClassList(classListId);
...
export async function loadTeacherClassListsByOwnerAny(ownerTeacherEmail: string) {
  const local = loadTeacherClassListsByOwner(ownerTeacherEmail);
  if (local.length > 0) return local;
  const cloud = await cloudLoadTeacherClassListsByOwner(ownerTeacherEmail);
  if (cloud.length > 0) {
    const current = loadTeacherClassLists();
    const map = new Map(current.map((item) => [item.id, item]));
    cloud.forEach((item) => {
      map.set(item.id, item);
    });
    saveTeacherClassLists(Array.from(map.values()));
  }
  return cloud;
}
