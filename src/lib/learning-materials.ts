...
export class LearningMaterialStorageError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "LearningMaterialStorageError";
  }
}
...
export function getMaterialCategoryLabel(category: MaterialCategory) {
  return MATERIAL_CATEGORY_OPTIONS.find((option) => option.value === category)?.label || "Tài liệu khác";
}
...
export function loadLearningMaterials() {
  ...
  try {
    const parsed = repairMojibakeDeep(JSON.parse(raw)) as LearningMaterial[];
...
export function saveLearningMaterials(materials: LearningMaterial[]) {
  ...
  try {
    ...
    if (text.toLowerCase().includes("quota") || text.toLowerCase().includes("exceeded")) {
      throw new LearningMaterialStorageError(
        "Bộ nhớ trình duyệt đã đầy. Hãy giảm số file đính kèm hoặc tải ít file hơn mỗi lần."
      );
    }
    throw error;
  }
...
export function getStudentVisibleMaterials(studentEmail: string) {
  ...
  return loadLearningMaterials()
    ...
    .filter((material) => material.assignedClassListIds.some((classId) => visibleClassIdSet.has(classId)))
    .sort((a, b) => b.updatedAt - a.updatedAt);
}
