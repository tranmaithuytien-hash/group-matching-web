...
function RoundSelector({
  ...
  const roundLabels: Record<GroupRoundKey, { title: string; description: string }> = {
    ...
    round4: {
      ...
      description: "Tự động fill c�c bạn chưa v�o nh�m để ho�n tất game."
    }
  };
      ...
      {(["round1", "round2", "round3", "round4"] as GroupRoundKey[]).map((roundKey) => {
        ...
        return (
          <label key={roundKey} className={locked ? "group-round-card locked" : "group-round-card"}>
            ...
            {locked ? <small>B�t bu�c</small> : <small>T�y ch�n</small>}
          </label>
        );
      })}
    </div>
...
export default function GroupMatchingCreatePage() {
  ...
  const handleSubmit = () => {
    ...
    if (participants.length < 2) {
      setStatusMessage("Danh s�ch cần tối thiểu 2 sinh vi�n để chia nh�m.");
      return;
    }
    ...
    const actionLogEntry = {
      ...
      at: now,
      action: currentEditingSession ? "S�a c�u h�nh game" : "T�o game m�i",
      ...
      <main className="section-page">
        <div className="site-shell group-shell">
          <div className="section-head section-head-single">
            <div>
              <span className="section-eyebrow">{isEditMode ? "S�a phi�n chia nh�m" : "T�o m�i chia nh�m"}</span>
              <h1>{isEditMode ? "C�p nh�t c�u h�nh game chia nh�m." : "T�o game chia nh�m cho l�p c�a b�n."}</h1>
            </div>
            <p>
              Bạn c� thể d�ng danh s�ch lớp đ� c� hoặc import mới trực tiếp. Sau khi tạo xong, hệ thống cấp class code
          ...
          <section className="group-form-card">
            ...
            <label className="field">
              ...
              />
              <small className="hint-copy">
                Nếu kh�ng c� cột thứ hạng, hệ thống tự đ�nh số theo thứ tự d�ng khi import.
              </small>
            </label>

            <div className="group-action-row">
              <button type="button" className="hero-primary" onClick={handleSubmit}>
                {isEditMode ? "L�u v� quay l�i qu�n l�" : "T�o phi�n v� b�t �u qu�n l�"}
              </button>
