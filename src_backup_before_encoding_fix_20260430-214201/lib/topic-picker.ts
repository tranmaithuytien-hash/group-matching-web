...
function normalizeSession(raw: Partial<TopicSession>) {
  return {
    ...
    type1PenaltyPoints: Math.max(0, Number(raw.type1PenaltyPoints) || 30),
...
export function resetTopicSessionById(sessionId: string) {
  ...
  const reset: TopicSession = normalizeSession({
    ...
    actionLogs: [{ at: Date.now(), action: "Reset game", detail: "X�a dữ liệu lượt cũ, đưa phi�n về trạng th�i chuẩn bị" }]
  });
...
export function getTopicSessionByRouteKey(sessionKey: string) {
  ...
  const byId = loadTopicSessions().find((session) => session.id === clean) || null;
  if (byId) return byId;
  const normalizedCode = clean.toUpperCase();
  return loadTopicSessions().find((session) => session.classCode === normalizedCode) || null;
}

export function getTopicSessionByCode(classCode: string) {
  const normalized = classCode.trim().toUpperCase();
  return loadTopicSessions().find((session) => session.classCode === normalized) || null;
}
...
export function randomFillType1(session: TopicSession) {
  ...
  remain.forEach((studentId) => {
    const target = groups.find(
      (group) =>
        !lockedGroupIds.has(group.id) && group.memberStudentIds.length < getGroupTargetSize(session, group.id)
    );
    if (target) {
    ...
    groups.push({
      ...
      memberNames: mapGroupNames(session, [studentId]),
      topic: ""
    });
  });
...
export function startBiddingType1(session: TopicSession) {
  ...
  return upsertTopicSession({
    ...
    type1Stage: "bidding",
    biddingRound: 1,
    type1RoundEndsAt: Date.now() + 5 * 60 * 1000,
    bidRecords: [],
    groupRemainingPoints: remaining,
    type1BidDraftByGroupId: {},
    type1BidApprovalsByGroupId: {},
    type1PenaltyPoints: Math.max(0, Number(session.type1PenaltyPoints) || 30),
    type1PenalizedGroupIds: [],
    type1TopRemainingGroupId: "",
    updatedAt: Date.now()
  });
...
export function resolveBiddingRound(session: TopicSession) {
  ...
  const penalty = Math.max(0, Number(session.type1PenaltyPoints) || 30);
  const finishWithRandom = (inputFinal: TopicGroup[], reason: string) => {
    ...
    const remainAfterRound = { ...nextRemain };
    const penalizedGroupIds = unassignedGroups.map((group) => group.id);
    penalizedGroupIds.forEach((groupId) => {
      remainAfterRound[groupId] = Math.max(0, (remainAfterRound[groupId] || 0) - penalty);
    });
  ...
  if (nextFinal.length >= targetCount || round >= 5) {
    ...
    return finishWithRandom(nextFinal, "Hết 5 v�ng đấu gi�, random chủ đề c�n lại");
  }
...
export function finalizeType1BiddingWithRandom(session: TopicSession) {
  ...
  const nextRemain = { ...(session.groupRemainingPoints || {}) };
  const penalizedGroupIds = remainingGroups.map((group) => group.id);
  penalizedGroupIds.forEach((groupId) => {
    nextRemain[groupId] = Math.max(0, (nextRemain[groupId] || 0) - penalty);
  });
  const ranking = workingGroups
    .map((group) => ({ id: group.id, remain: nextRemain[group.id] || 0 }))
    .sort((a, b) => b.remain - a.remain || a.id.localeCompare(b.id));
  return upsertTopicSession(
    appendTopicSessionLog(
      ...
      "Kết th�c đấu gi�",
      "GV kết th�c đấu gi�, random chủ đề c�n lại"
    )
  );
}
