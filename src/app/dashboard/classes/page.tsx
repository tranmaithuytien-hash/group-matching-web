...
export default function ClassesPage() {
  ...
  const refreshClassLists = async (ownerEmail: string) => {
    if (!ownerEmail) {
      setClassLists([]);
      return;
    }
    ...
    const local = loadTeacherClassListsByOwner(ownerEmail);
    if (local.length > 0) {
      setClassLists(local);
      return;
    }
    const cloud = await loadTeacherClassListsByOwnerAny(ownerEmail);
    setClassLists(cloud);
  };
  ...
  const applyParsedSheet = (mode: SheetMode, rawText: string) => {
    ...
    if (parsed.students.length === 0) {
      if (mode === "create") {
        setStatusMessage("Không đọc được dữ liệu từ nội dung đã dán.");
      } else {
        setModalStatus("Không đọc được dữ liệu từ nội dung đã dán.");
      }
      return;
    }
    ...
    if (mode === "create") {
      setStatusMessage("Đã dán dữ liệu từ Excel vào bảng.");
    } else {
      setModalStatus("Đã dán dữ liệu từ Excel vào bảng.");
    }
  };
  ...
  const pasteFromExcel = async (mode: SheetMode) => {
    ...
    const manualText = window.prompt("Dán bảng dữ liệu từ Excel vào đây:");
    if (!manualText) {
      return;
    }
  ...
  const handleExcelFileChange = async (mode: SheetMode, event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Không thể đọc file Excel. Vui lòng kiểm tra định dạng file.";
      if (mode === "create") {
  ...
  const saveCreateClassList = () => {
    ...
    if (students.length === 0) {
      setStatusMessage("Danh sách lớp chưa có dòng hợp lệ (cần tối thiểu MSSV và Họ tên).");
      return;
    }
    ...
    if (!saved) {
      setStatusMessage("Không thể lưu danh sách lớp. Vui lòng thử lại.");
      return;
    }

    setStatusMessage("Đã tạo danh sách lớp mới.");
    resetCreateSheet();
    void refreshClassLists(teacherEmail);
  };
  ...
  const saveModalClassList = () => {
    ...
    if (students.length === 0) {
      setModalStatus("Danh sách lớp chưa có dòng hợp lệ (cần tối thiểu MSSV và Họ tên).");
      return;
    }
  ...
  const deleteClass = (classList: TeacherClassList) => {
    const confirmed = window.confirm(`Bạn có chắc muốn xóa danh sách lớp "${classList.className}"?`);
    if (!confirmed) {
      return;
    }
    ...
    setStatusMessage("Đã xóa danh sách lớp.");
    void refreshClassLists(teacherEmail);
  };
                    ...
                    <div className="class-col-head">
                      ...
                      <div className="class-col-head-actions">
                        <button
                          ...
                          aria-label={`Di chuyển ${column.label} sang trái`}
        ...
        <div className="site-shell group-shell">
          <div className="section-head section-head-single">
            <div>
              ...
              <h1 className="group-manage-page-title">Quản lý danh sách lớp đã import.</h1>
            </div>
            <p>
              Tại đây bạn có thể tạo mới, xem, sửa hoặc xóa danh sách lớp. Các lớp đã import từ phiên game
              cũng sẽ tự động lưu về trang này.
            </p>
          </div>

          <section className="group-form-card">
            <h2>Import danh sách lớp mới</h2>
            ...
            <div className="field">
              ...
              {createImportMode === "manual" ? (
                ...
                <div className="class-import-excel">
                  ...
                  {excelFileName ? <small className="hint-copy">Đã chọn: {excelFileName}</small> : null}
                </div>
              )}
          ...
          <section className="content-card">
            <div className="group-action-row classes-list-head-row">
              <h2>Danh sách lớp ({classLists.length})</h2>
            </div>

            {classLists.length === 0 ? (
              <p>Chưa có danh sách lớp nào trong tài khoản này.</p>
            ) : (
                ...
                {classLists.map((classList) => (
                  <article key={classList.id} className="classes-list-card">
                    ...
                    <p className="hint-copy">
                      {classList.sourceSessionId ? "Nguồn: import từ phiên game" : "Nguồn: import thủ công"}
                    </p>
          ...
          <article className="group-modal-card class-view-modal">
            <div className="group-modal-head">
              <div>
                ...
                <p>{(isModalEditing ? modalRows : viewingClass.students).length} sinh viên</p>
              </div>
              <button
                type="button"
                className="hero-secondary"
