...
import {
  ...
  findTeacherAccountByEmailAny,
  registerTeacherAccountAny,
  verifyTeacherCredentials,
  verifyTeacherCredentialsAny
...
function AuthNotice({
  ...
  return (
    <p className={notice.type === "error" ? "auth-notice auth-notice-error" : "auth-notice auth-notice-success"}>
      <span>{notice.type === "error" ? "Lỗi:" : "Thông báo:"}</span> {notice.message}
    </p>
  );
}
...
export default function LoginPage() {
  ...
  const executeGoogleLogin = async () => {
    ...
    try {
      ...
      if (!email) {
        setErrorNotice("Không lấy được email từ tài khoản Google. Vui lòng thử tài khoản khác.");
        return null;
      }
  ...
  const handleTeacherLogin = async () => {
    ...
    const account =
      verifyTeacherCredentials(teacherEmail, teacherPassword) ||
      (await verifyTeacherCredentialsAny(teacherEmail, teacherPassword));
    if (!account) {
  ...
  const handleTeacherRegister = async () => {
    ...
    if (teacherPassword !== teacherConfirmPassword) {
      setErrorNotice("Mật khẩu xác nhận chưa khớp.");
      return;
    }

    const registerResult = await registerTeacherAccountAny({
  ...
  const handleTeacherGoogle = async () => {
    ...
    let account = findTeacherAccountByEmail(email) || (await findTeacherAccountByEmailAny(email));
    if (!account) {
      const autoRegister = await registerTeacherAccountAny({
        email,
      ...
      if (autoRegister.ok) {
        ...
        account = findTeacherAccountByEmail(email) || (await findTeacherAccountByEmailAny(email));
      }
    }
