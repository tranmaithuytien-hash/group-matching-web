...
import { uploadMaterialAttachmentsIfNeeded } from "../../../../lib/material-storage-cloud";
import {
...
function materialPreviewText(material: LearningMaterial) {
  ...
  if (material.attachments?.[0]) return `Tệp: ${material.attachments[0].name}`;
...
export default function TopicPickerCreatePage() {
  ...
  const [previewOpenUrl, setPreviewOpenUrl] = useState("");
  ...
  useEffect(() => {
    ...
    void (async () => {
      const classesLocal = loadTeacherClassListsByOwner(teacherEmail);
      const classes = classesLocal.length > 0 ? classesLocal : await loadTeacherClassListsByOwnerAny(teacherEmail);
  ...
  const handleSaveUploadedTopic = async () => {
    if (!teacherEmail) return;
    ...
    const uploadedAttachments = await uploadMaterialAttachmentsIfNeeded(
      teacherEmail,
      materialId,
  ...
  const handleConfirmNewClassInput = () => {
    ...
    if (parsed.students.length === 0) {
      return setStatusMessage("Không đọc được danh sách lớp. Hãy nhập theo định dạng: MSSV[TAB]Họ và tên[TAB]Email[TAB]Điểm.");
    }
    ...
    if (!saved) return setStatusMessage("Không thể xác nhận danh sách lớp. Vui lòng thử lại.");
    ...
    setStatusMessage(`Đã xác nhận lớp "${saved.className}" với ${saved.students.length} sinh viên.`);
  };
  ...
  const handleSaveSession = () => {
    ...
    payload = appendTopicSessionLog(
      payload,
      existing ? "Cập nhật cấu hình phiên" : "Tạo phiên mới",
    ...
    <main className="section-page">
      <div className="site-shell group-shell">
        <div className="section-head section-head-single">
          <div>
            <span className="section-eyebrow">{editId ? "Sửa phiên" : "Tạo phiên"}</span>
            <h1>{editId ? "Cập nhật cấu hình phiên lựa chọn chủ đề" : "Cấu hình phiên lựa chọn chủ đề thuyết trình"}</h1>
          </div>
        </div>
        ...
        <section className="group-form-card">
          <div className="field">
            ...
            <select className="text-input" value={selectedSubjectCode} onChange={(e) => { setSelectedSubjectCode(e.target.value); setSubjectName(e.target.value); }}>
              {subjectOptions.length === 0 ? <option value="">Chưa có môn trong Tài liệu</option> : null}
          ...
          {subjectMode === "upload" ? (
            <article className="content-card" style={{ marginBottom: 16 }}>
              ...
              <label className="field">
                ...
                {uploadFile ? <small className="hint-copy">Đã chọn file: {uploadFile.name}</small> : null}
              </label>
          ...
          {classSourceMode === "existing" ? (
            ...
            <>
              ...
              <label className="field">
                <span>Danh sách SV (nhập thủ công)</span>
                <textarea
          ...
          <div className="field">
            ...
            {candidateTopics.length === 0 ? <p className="hint-copy">Chưa có chủ đề trong Tài liệu cho môn này.</p> : null}
                        ...
                        {item.externalUrl ? (
                          <button
                            ...
                            onClick={(event) => {
                              ...
                              const openUrl = normalizeExternalUrl(item.externalUrl || "");
                              if (!openUrl) return;
                              setPreviewTitle(item.title);
                              setPreviewUrl(toEmbeddableUrl(openUrl));
          ...
          <label className="field">
            ...
            <select className="text-input" value={String(typeValue)} onChange={(e) => setTypeValue(Number(e.target.value) as TopicPickerType)}>
              <option value="1">Kiểu 1 - Đấu giá chủ đề</option>
          ...
          <div className="group-action-row">
            <button type="button" className="hero-primary" onClick={handleSaveSession}>{editId ? "Lưu thay đổi" : "Tạo phiên"}</button>
      ...
      {previewUrl ? (
        <div className="group-modal-overlay" onClick={() => setPreviewUrl("")}>
          <article className="group-modal-card materials-preview-modal" onClick={(event) => event.stopPropagation()}>
            <div className="materials-preview-head">
              <h3>{previewTitle || "Xem preview"}</h3>
              <a className="hero-secondary" href={previewOpenUrl || previewUrl} target="_blank" rel="noreferrer">
                Mở tab mới
              </a>
              <button type="button" className="hero-secondary" onClick={() => setPreviewUrl("")}>
                Đóng
              </button>
            </div>
            <div className="materials-preview-content">
              <iframe title={previewTitle || "Preview"} src={previewUrl} className="materials-preview-frame" />
            </div>
          </article>
        </div>
      ) : null}
