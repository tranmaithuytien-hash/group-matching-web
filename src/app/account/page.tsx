...
import { DEMO_SESSION_KEY, getDemoSession, type DemoSession, updateTeacherAccount } from "../../lib/auth-storage";
...
export default function AccountPage() {
  ...
  const handleAvatarSelected = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_AVATAR_SIZE) {
      setStatusType("error");
      setStatusMessage("Ảnh vượt quá 1MB. Vui lòng chọn ảnh nhỏ hơn.");
      event.target.value = "";
      return;
    }
          ...
          <div className="account-profile-card">
            <div className="account-profile-grid">
              <div className="account-profile-main">
                ...
                <label className="account-profile-field">
                  <span>Ngày sinh</span>
                  <input className="text-input" type="date" value={birthDate} onChange={(event) => setBirthDate(event.target.value)} />
                </label>
                ...
                {statusType !== "idle" ? (
                  <p className={statusType === "success" ? "account-status success" : "account-status error"}>{statusMessage}</p>
                ) : null}
              ...
              <aside className="account-profile-aside">
                <div className="account-avatar-preview">
                  {avatarUrl ? <img src={avatarUrl} alt="Ảnh đại diện" /> : <span>{(displayName || username || "U").slice(0, 1).toUpperCase()}</span>}
                </div>
                <input
                  ref={avatarInputRef}
                  type="file"
